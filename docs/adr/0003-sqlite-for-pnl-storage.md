...
| **DuckDB** | No foreign key enforcement or schema constraints — the relational integrity guarantees that SQLite provides are absent. A strong alternative if the dataset grows to where columnar compression or vectorised execution matters, but not at current data volumes and not without a substitute for schema enforcement. |
